...
def monte_carlo_utilities(game_state, player, max_simulations=500):
...
def monte_carlo_simulate(game_state, player, chosen_card):
    utility = 0
    player_names = [p.name for p in game_state.players]
    # TODO: this is cheating
    hands = {player.name: player.cards for player in game_state.players}
    ...
    suit_led = game_state.trick.suit_led or card.suit
    for player_name in game_state.trick.left_to_play:
        if player_name != player.name:
            card_to_play = random.choice(get_legal_cards(hands[player_name], suit_led))
    ...
    while hands[game_state.players[first_to_play].name]:
        ...
        played = dict{lead_card: first_player_name}
